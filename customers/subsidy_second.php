<?php
// customers/subsidy_second.php — Customers who received Second Subsidy
...
$where  = "WHERE i.subsidy_2nd_status = 'Yes'";
$params = [];
...
try {
    $sql = "
        SELECT c.id, c.operator_name, c.group_name, c.name, c.email, c.mobile,
               ...
               i.subsidy_1st_status, i.subsidy_2nd_status
        FROM customers c
        INNER JOIN installations i ON c.id = i.customer_id
        LEFT JOIN payments p ON c.id = p.customer_id
        $where
...
function sf2Url(array $ov = []): string {
    $p = array_merge([
    ...
    return '/APN-Solar/customers/subsidy_second.php?' . http_build_query(array_filter($p, fn($v) => $v !== ''));
}

$pageTitle = 'Received Subsidy Second';
...
.sub-badge-1{display:inline-block;background:#dbeafe;color:#1e40af;border-radius:20px;padding:2px 10px;font-size:.68rem;font-weight:700;}
...
<h2 style="font-size:1.1rem;font-weight:700;margin-bottom:12px;color:#1e293b;">
    <i class="fas fa-hand-holding-usd" style="color:#3b82f6;margin-right:7px;"></i>
    Customer List who received Second Subsidy:-
...
<div class="pl-filters">
    <form method="GET" action="/APN-Solar/customers/subsidy_second.php">
        ...
        <div style="display:flex;gap:6px;align-items:flex-end;">
            ...
            <a href="/APN-Solar/customers/subsidy_second.php" class="btn btn-reset"><i class="fas fa-undo"></i> Reset</a>
        </div>
    </form>
</div>

<div class="dt-bar">
    <button class="btn btn-excel" onclick="sf2Excel()"><i class="fas fa-file-excel"></i> Excel</button>
</div>

<div class="ctrl-bar">
    <div style="display:flex;align-items:center;gap:6px;font-size:.82rem;">
        <form method="GET" action="/APN-Solar/customers/subsidy_second.php" style="display:flex;align-items:center;gap:5px;">
    ...
    <div class="search-box"><span>Search:</span><input type="text" id="sf2Search" placeholder="Name, mobile, ID..." oninput="sf2Filter(this.value)" value="<?php echo htmlspecialchars($search); ?>"></div>
</div>

<div class="table-wrap">
    <div class="table-responsive">
        <table class="table" id="sf2Table">
            <thead>
                <tr>
                    <th onclick="sf2Sort(0)">Sr<br>No.</th>
                    <th onclick="sf2Sort(1)">Operator<br>Name</th>
                    <th onclick="sf2Sort(2)">Group Name</th>
                    <th onclick="sf2Sort(3)">Name</th>
                    <th onclick="sf2Sort(4)">Email</th>
                    <th onclick="sf2Sort(5)">Mobile</th>
                    <th onclick="sf2Sort(6)">District</th>
                    <th onclick="sf2Sort(7)">Electricity<br>Id</th>
                    <th onclick="sf2Sort(8)">Kw</th>
                    <th onclick="sf2Sort(9)" class="text-right">Total<br>Amount</th>
                    <th onclick="sf2Sort(10)" class="text-right">Due<br>Amount</th>
                    <th onclick="sf2Sort(11)">Subsidy<br>1st</th>
                    <th onclick="sf2Sort(12)">Subsidy<br>2nd</th>
                    <th onclick="sf2Sort(13)">Remarks</th>
                    <th onclick="sf2Sort(14)">Last Updated<br>on</th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody>
            <?php if ($customers):
                ...
                <tr>
                    ...
                    <td><?php echo $r['subsidy_1st_status']==='Yes'?'<span class="sub-badge-1"><i class="fas fa-check" style="font-size:.6rem;"></i> Yes</span>':'<span style="color:#94a3b8;font-size:.72rem;">No</span>'; ?></td>
                    ...
                    <td>
                        <a href="/APN-Solar/customers/customer_status_update.php?id=<?php echo (int)$r['id']; ?>&back=subsidy_second"
    ...
    <div class="pag-bar">
        ...
        <div class="pagination">
            <?php if($page>1): ?><a class="pg-btn" href="<?php echo sf2Url(['page'=>$page-1]); ?>">Prev</a><?php else: ?><span class="pg-btn disabled">Prev</span><?php endif; ?>
            <?php $ps=max(1,$page-3);$pe=min($totalPages,$page+3);
            if($ps>1){echo '<a class="pg-btn" href="'.sf2Url(['page'=>1]).'">1</a>';if($ps>2)echo '<span class="pg-btn disabled">…</span>';}
            for($p=$ps;$p<=$pe;$p++) echo '<a class="pg-btn '.($p===$page?'active':'').'" href="'.sf2Url(['page'=>$p]).'">'.$p.'</a>';
            if($pe<$totalPages){if($pe<$totalPages-1)echo '<span class="pg-btn disabled">…</span>';echo '<a class="pg-btn" href="'.sf2Url(['page'=>$totalPages]).'">'.$totalPages.'</a>';}?>
            <?php if($page<$totalPages): ?><a class="pg-btn" href="<?php echo sf2Url(['page'=>$page+1]); ?>">Next</a><?php else: ?><span class="pg-btn disabled">Next</span><?php endif; ?>
        </div>
    </div>
</div>

<script>
function sf2Filter(q){q=q.toLowerCase();document.querySelectorAll('#sf2Table tbody tr').forEach(r=>{r.classList.toggle('hidden-row',q!==''&&!r.innerText.toLowerCase().includes(q));});}
let sf2SC=-1,sf2SA=true;
function sf2Sort(c){const ths=document.querySelectorAll('#sf2Table thead th');ths.forEach(t=>t.classList.remove('sort-asc','sort-desc'));if(sf2SC===c){sf2SA=!sf2SA;}else{sf2SC=c;sf2SA=true;}ths[c].classList.add(sf2SA?'sort-asc':'sort-desc');const tb=document.querySelector('#sf2Table tbody');const rows=[...tb.querySelectorAll('tr')].filter(r=>!r.querySelector('[colspan]'));rows.sort((a,b)=>{const va=a.cells[c]?.innerText.replace(/[₹,]/g,'').trim()||'',vb=b.cells[c]?.innerText.replace(/[₹,]/g,'').trim()||'';const na=parseFloat(va),nb=parseFloat(vb);const cm=(!isNaN(na)&&!isNaN(nb))?na-nb:va.localeCompare(vb);return sf2SA?cm:-cm;});rows.forEach(r=>tb.appendChild(r));}
function sf2Excel(){const rows=document.querySelectorAll('#sf2Table tr');let html='<table border="1"><thead>';let inB=false;rows.forEach(row=>{const isH=row.closest('thead');if(isH&&!inB)html+='<tr>';else if(!isH&&!inB){html+='</thead><tbody><tr>';inB=true;}else html+='<tr>';row.querySelectorAll('th,td').forEach((c,i)=>{if(i===15)return;html+=`<${isH?'th':'td'}>${c.innerText.trim()}</${isH?'th':'td'}>`;});html+='</tr>';});html+='</tbody></table>';const b=new Blob([html],{type:'application/vnd.ms-excel;charset=utf-8;'});const a=Object.assign(document.createElement('a'),{href:URL.createObjectURL(b),download:'subsidy_second_'+new Date().toISOString().slice(0,10)+'.xls'});a.click();URL.revokeObjectURL(a.href);}
</script>
