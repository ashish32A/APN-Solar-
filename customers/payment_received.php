<?php
// customers/payment_received.php — Payment Received List
...
$where  = "WHERE p.payment_received > 0";
$params = [];
...
function prUrl(array $ov = []): string {
    ...
    return '/APN-Solar/customers/payment_received.php?' . http_build_query(array_filter($p, fn($v) => $v !== ''));
}

$pageTitle = 'Payment Received List';
...
.btn { display:inline-flex; align-items:center; gap:5px; padding:6px 14px; border-radius:6px; font-size:.8rem; font-weight:600; cursor:pointer; border:none; font-family:inherit; transition:all .15s; text-decoration:none; white-space:nowrap; }
...
.table thead th { background:#f0f4f8; padding:8px 10px; font-weight:700; font-size:.7rem; text-transform:uppercase; letter-spacing:.04em; color:#64748b; border-bottom:2px solid #e2e8f0; white-space:nowrap; text-align:left; cursor:pointer; user-select:none; position:sticky; top:0; z-index:1; }
...
.recv-badge { display:inline-block; background:#dcfce7; color:#15803d; border-radius:20px; padding:2px 9px; font-size:.7rem; font-weight:700; }
...
.pg-btn { display:inline-flex; align-items:center; justify-content:center; min-width:32px; height:30px; padding:0 8px; border:1.5px solid #e2e8f0; border-radius:6px; background:#fff; color:#3b82f6; font-size:.78rem; font-weight:600; cursor:pointer; text-decoration:none; transition:all .15s; }
...
<h2 style="font-size:1.15rem;font-weight:700;margin-bottom:12px;color:#1e293b;">
    <i class="fas fa-check-circle" style="color:#15803d;margin-right:7px;"></i>
...
<div class="summary-cards">
    ...
    <div class="s-card">
        ...
        <div>
            ...
            <div class="s-val" style="color:#15803d;">₹<?php echo number_format((float)($totals['received'] ?? 0), 2); ?></div>
        </div>
    </div>
    <div class="s-card">
...
<div class="pl-filters">
    <form method="GET" action="/APN-Solar/customers/payment_received.php">
        <div class="pl-fg">
        ...
        <div style="display:flex;gap:6px;align-items:flex-end;">
            ...
            <a href="/APN-Solar/customers/payment_received.php" class="btn btn-reset"><i class="fas fa-undo"></i> Reset</a>
        </div>
    </form>
</div>
...
<!-- Export -->
<div class="dt-bar">
    <button class="btn btn-excel" onclick="prExportExcel()"><i class="fas fa-file-excel"></i> Excel</button>
</div>

<!-- Controls -->
<div class="ctrl-bar">
    <div style="display:flex;align-items:center;gap:8px;font-size:.82rem;">
        <form method="GET" action="/APN-Solar/customers/payment_received.php" style="display:flex;align-items:center;gap:6px;">
    ...
    <div class="search-box">
        <span>Search:</span>
        <input type="text" id="prSearch" placeholder="Name, mobile, ID..."
               oninput="prFilter(this.value)" value="<?php echo htmlspecialchars($search); ?>">
    </div>
</div>
...
<div class="table-wrap">
    <div class="table-responsive">
        <table class="table" id="prTable">
            <thead>
                <tr>
                    <th onclick="prSort(0)">Sr<br>No.</th>
                    <th onclick="prSort(1)">Operator<br>Name</th>
                    <th onclick="prSort(2)">Group<br>Name</th>
                    <th onclick="prSort(3)">Name</th>
                    <th onclick="prSort(4)">Mobile</th>
                    <th onclick="prSort(5)">District</th>
                    <th onclick="prSort(6)">Electricity<br>Id</th>
                    <th onclick="prSort(7)">KW</th>
                    <th onclick="prSort(8)" class="text-right">Total<br>Amount</th>
                    <th onclick="prSort(9)" class="text-right">Received</th>
                    <th onclick="prSort(10)" class="text-right">Due<br>Amount</th>
                    <th onclick="prSort(11)">Reg<br>Date</th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody>
            <?php if (count($customers) > 0):
                ...
                <tr>
                    ...
                    <td class="text-right">
                        <span class="recv-badge">₹<?php echo number_format((float)($row['payment_received'] ?? 0), 2); ?></span>
                    </td>
                    <td class="text-right <?php echo (float)($row['due_amount']??0) > 0 ? 'text-danger' : ''; ?>">
                        ₹<?php echo number_format((float)($row['due_amount'] ?? 0), 2); ?>
                    </td>
                    <td style="white-space:nowrap;font-size:.73rem;"><?php echo htmlspecialchars(substr($row['created_at'] ?? '', 0, 10)); ?></td>
                    <td>
                        <div class="action-col">
                            <a href="/APN-Solar/customers/edit.php?id=<?php echo (int)$row['id']; ?>&back=payment_received"
                               class="btn btn-edit btn-sm"><i class="fas fa-pencil-alt"></i> Edit</a>
                            <a href="/APN-Solar/customers/customer_status_view.php?id=<?php echo (int)$row['id']; ?>&back=payment_received"
                ...
                <tr><td colspan="13" style="text-align:center;padding:40px;color:#94a3b8;">
                    <i class="fas fa-rupee-sign" style="font-size:2rem;display:block;margin-bottom:8px;"></i>
                    No payment received records found<?php echo ($group || $district || $search || $regFrom || $regTo) ? ' matching your filters.' : '.'; ?>
                </td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    ...
    <div class="pag-bar">
        ...
        <div class="pagination">
            <?php if ($page > 1): ?>
                <a class="pg-btn" href="<?php echo prUrl(['page' => $page - 1]); ?>">Previous</a>
            <?php else: ?>
            ...
            if ($ps > 1): ?>
                <a class="pg-btn" href="<?php echo prUrl(['page'=>1]); ?>">1</a>
                <?php if ($ps > 2): ?><span class="pg-btn disabled">…</span><?php endif;
            endif;
            for ($p=$ps; $p<=$pe; $p++): ?>
                <a class="pg-btn <?php echo $p===$page ? 'active':''; ?>"
                   href="<?php echo prUrl(['page'=>$p]); ?>"><?php echo $p; ?></a>
            <?php endfor;
            if ($pe < $totalPages):
                if ($pe < $totalPages-1): ?><span class="pg-btn disabled">…</span><?php endif; ?>
                <a class="pg-btn" href="<?php echo prUrl(['page'=>$totalPages]); ?>"><?php echo $totalPages; ?></a>
            <?php endif; ?>

            <?php if ($page < $totalPages): ?>
                <a class="pg-btn" href="<?php echo prUrl(['page' => $page + 1]); ?>">Next</a>
            <?php else: ?>
...
let prSortCol = -1, prSortAsc = true;
function prSort(col) {
    const ths = document.querySelectorAll('#prTable thead th');
    ths.forEach(th => th.classList.remove('sort-asc','sort-desc'));
    if (prSortCol === col) { prSortAsc = !prSortAsc; } else { prSortCol = col; prSortAsc = true; }
    ths[col].classList.add(prSortAsc ? 'sort-asc' : 'sort-desc');
...
function prExportExcel() {
    const rows = document.querySelectorAll('#prTable tr');
    ...
    const a = Object.assign(document.createElement('a'), {
        ...
        download: 'payment_received_' + new Date().toISOString().slice(0,10) + '.xls'
    });
